...
export default function ViewProductPage() {
  ...
  if (isLoading) return <div className="min-h-screen flex items-center justify-center font-bold text-slate-400">Loading Product...</div>;
  ...
  return (
    <div className="min-h-screen bg-white pb-20">
      ...
      <div className="w-full mx-auto px-4 py-6">
        <button onClick={() => window.history.back()} className="flex items-center text-sm font-bold text-slate-500 hover:text-slate-900 transition-colors group">
  ...
  {isOutOfStock ? "• Out of Stock" : "• In Stock"}
            ...
            <div className="pt-8 order-2">
              <div className="flex items-baseline gap-4">
                ...
                <span className="text-xl text-slate-300 line-through">₹{Number(product.price) + 200}</span>
              </div>
            </div>

            {/* 3. 🛒 Action Buttons (Mobile: Order 3 | Desktop: Order 4) */}
           

            {/* 4. 📖 Description Section (Mobile: Order 4 | Desktop: Order 3) */}
  ...
  {isOutOfStock ? "OUT OF STOCK" : "ADD TO CART"}
                ...
                <div className="w-full h-full bg-[#2A4150] rounded-xl overflow-hidden shadow-lg">
                  <QuantitySelector
                    ...
                    onIncrease={() => {
                      if (cartItem.quantity >= product.stock) return alert("Out of stock!");
                      dispatch(updateQty({ id: product.id, delta: 1 }));
                    }}
                    onDecrease={() => {
                      if (cartItem.quantity === 1) dispatch(removeFromCart(product.id));
                ...
                <div className="absolute -top-3 left-8 bg-white px-4 py-1 border border-slate-100 rounded-full flex items-center gap-2 shadow-sm">
                  ...
                  <span className="text-[10px] font-bold uppercase tracking-widest text-slate-500">Product Highlights</span>
                </div>
                <div
            ...
            <div className="mt-6 lg:mt-10 grid grid-cols-2 gap-4 order-5">
              <div className="flex items-center gap-3 p-4 rounded-2xl border border-slate-100 bg-white">
                ...
                <p className="text-xs font-bold text-slate-900 leading-tight">Free Express Delivery</p>
              </div>
              <div className="flex items-center gap-3 p-4 rounded-2xl border border-slate-100 bg-white">
                <ShieldCheck className="text-emerald-600" size={24} />
                <div>
                  <p className="text-xs font-bold text-slate-900 leading-tight">100% Authentic</p>
                  <p className="text-[10px] text-slate-400">Direct Brand Origin</p>
                </div>
              </div>
            </div>
